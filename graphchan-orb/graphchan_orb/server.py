...
def _configure(params: dict[str, Any]) -> dict[str, Any]:
    ...
    _settings = params.get("settings", {})
    api_url = _settings.get("api_url", "").strip()
    if api_url:
        _client = GraphchanClient(base_url=api_url)
    else:
        _client = None
...
def _poll_events(_params: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in recent:
        ...
        # Skip posts made by this agent (identified by metadata)
        metadata = post.get("metadata") or {}
        agent_info = metadata.get("agent") or {}
        # We don't know the agent username here; the backend handles dedup
        # by author matching — so we just return all posts and let the host filter.
